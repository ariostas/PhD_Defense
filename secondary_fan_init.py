...
class SecondaryFanInit(ThreeDScene):
    def construct(self):
        ...
        # I had to split this into multiple scenes because there is an apparent
        # bug where the lines appear on top of the dots even after setting z_indices.
        # Splitting them fixes this for no apparent reason
        
        self.next_section("SecondaryFan 0", PresentationSectionType.SKIP)
        self.play(*[FadeIn(o) for o in [d_fan, dots]+poly_lines])
        self.next_section("SecondaryFan 1", PresentationSectionType.SKIP)
        d_fan.add_updater(init_lift)
        self.add(proj_lines, vert_lines, lifted_lines)
        self.wait(self.lifttime)