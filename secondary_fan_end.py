...
class SecondaryFanEnd(ThreeDScene):
    def construct(self):
        ...
        d_fan = Dot(point=axes2d.c2p(0,1), color=BLACK)
        self.lifttime = 1
        self.runtime = 5
        self.t_lift = 0
        self.t_anim = PI/2
        ...
        self.curr_lines = [list(l) for l in curr_lines]
        dots = get_dots()
        proj_lines = get_proj_lines()
        vert_lines = get_vert_lines()
        lifted_lines = get_lifted_lines()
        self.add(d_fan, dots,*poly_lines,proj_lines, vert_lines, lifted_lines)
        self.next_section("SecondaryFan 3", PresentationSectionType.SKIP)
        self.play(*[FadeOut(o) for o in [d_fan, cone1, cone2, cone3, cone4, cone5, dots, proj_lines, vert_lines, lifted_lines]+poly_lines])